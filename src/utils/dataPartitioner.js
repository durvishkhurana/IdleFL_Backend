...
/**
 * Converts a CSV file into numeric features and targets.
 *
 * @param {string} datasetPath
 * @returns {Promise<{ rows: Array<{ X: number[], y: number }>, totalRows: number }>}
 */
async function loadTabularDataset(datasetPath) {
  let rawCsv

  try {
    rawCsv = await readFile(datasetPath, 'utf8')
  } catch (error) {
    throw new Error(`Unable to read dataset file "${datasetPath}": ${error.message}`)
  }
  ...
  if (lines.length < 2) {
    throw new Error(`Malformed CSV "${path.basename(datasetPath)}": expected a header row and at least one data row`)
  }

  const header = parseCsvLine(lines[0])
  if (header.length < 2) {
    throw new Error(`Malformed CSV "${path.basename(datasetPath)}": expected at least one feature column and one target column`)
  }
  ...
  const rows = lines.slice(1).map((line, lineIndex) => {
    ...
    if (cells.length !== header.length) {
      throw new Error(
        `Malformed CSV "${path.basename(datasetPath)}" on data row ${lineIndex + 2}: expected ${header.length} columns, received ${cells.length}`
      )
    }
    ...
    if (invalidIndex !== -1) {
      throw new Error(
        `Malformed CSV "${path.basename(datasetPath)}" on data row ${lineIndex + 2}: column "${header[invalidIndex]}" is not numeric`
      )
    }
 ...
 *
 * @param {Object} opts
 * @param {string} opts.datasetPath
 * @param {string} opts.modelType
 * @param {number} opts.shardStart
 * @param {number} opts.shardEnd
 * @returns {Promise<Object>}
 */
export async function getShardPayload({ datasetPath, modelType, shardStart, shardEnd }) {
  if (modelType === 'CNN') {
  ...
  const { rows } = await loadTabularDataset(datasetPath)
 ...
 */
export async function partitionDataset({ datasetPath, devices, modelType }) {
  ...
  const { rows, totalRows } = await loadTabularDataset(datasetPath)
