...
async def run_agent():
    ...
    async def on_task_assigned(data):
        ...
        print(
            f"[DEBUG] globalWeights present: {bool(config.get('globalWeights') or config.get('global_weights'))}"
        )
        gw = config.get("globalWeights") or config.get("global_weights")
        print(f"[DEBUG] globalWeights length: {len(gw) if gw else 0}")
